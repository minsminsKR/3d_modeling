...
class ViewProcessor:
    ...
    def bake_view_selection(
        ...
        selection_resolution = getattr(self.config, "view_selection_resolution", 1024)
        self.render.set_default_render_resolution(selection_resolution)
    ...
    def bake_from_multiview(self, views, camera_elevs, camera_azims, view_weights):
        ...
        for view, camera_elev, camera_azim, weight in zip(views, camera_elevs, camera_azims, view_weights):
            ...
            if torch.count_nonzero(project_cos_map) == 0:
                continue
        ...
        if not project_textures:
            texture = torch.zeros(self.render.texture_size + (3,), device=self.render.device)
            ori_trust_map = torch.zeros(self.render.texture_size + (1,), device=self.render.device)
