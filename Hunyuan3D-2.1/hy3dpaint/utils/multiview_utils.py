...
class multiviewDiffusionNet:
    def __init__(self, config) -> None:
        ...
        local_dir = getattr(config, "multiview_local_dir", None)
        local_model_path = None
        if local_dir:
            local_model_path = os.path.join(local_dir, "hunyuan3d-paintpbr-v2-1")
            if not os.path.exists(os.path.join(local_model_path, "model_index.json")):
                ...
                if not os.path.exists(os.path.join(local_model_path, "model_index.json")):
                    raise FileNotFoundError(f"Missing model_index.json in local texture model directory: {local_model_path}")
            ...
            model_path = huggingface_hub.snapshot_download(**snapshot_kwargs)
