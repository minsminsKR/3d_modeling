...
- 맵 구조를 계속 검수해야 하므로 기본 밝기는 후레쉬 없이도 벽, 바닥, 계단, 문 윤곽이 보이도록 높게 잡혀 있습니다.
...
브라우저 자동 검증 스크립트는 초기화, 매미 복도용 HorrorEvent manager와 오브제, 런타임 사운드 비활성화, 벽/바닥/천장/계단/문/캐비넷 텍스처 적용, 미닫이문 패널 이동, 모든 열리는 문의 실제 방 연결, 2층 높이/계단, locked/blocked door, 계단 waypoint 기반 층간 추격/순찰, stuck 방지, 두꺼운 2층 슬래브, 층별 walkable/blocked/void/drop zone 규칙, 등 뒤 걷기/달리기 감지, 층간 포획 방지, 캐비넷 탈출 시점, 방/층을 오가는 전역 순찰, 문을 통한 추격/배회, 캐비넷 조사 중 Run 모션 유지, Cyclopse 지면 접지, 일시정지, 열쇠/클리어, 캐비넷 생존/사망 흐름을 확인합니다.
