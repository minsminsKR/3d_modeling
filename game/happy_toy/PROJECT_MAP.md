...
| `src/core/Game.js` | 렌더러, 장면, 맵, 플레이어, 적, HUD, 루프를 조립하는 최상위 모듈. 열쇠 수, 클리어 상태, 일시정지, 캐비넷 이벤트도 여기서 총괄 | 큰 시스템을 추가하거나 연결할 때 |
...
현재 적은 `Walking.fbx`의 첫 애니메이션을 기본 순찰 루프로 재생합니다. 추격 상태에서는 `gameConfig.js`의 `animationUrls.chase`에 지정한 Mixamo FBX를 재생합니다. 캐릭터 텍스처는 `model_test`와 같은 방식으로 조명에 묻히지 않는 `MeshBasicMaterial`에 적용합니다. Mixamo 보행 FBX의 `Hips.position` root motion drift는 루프 리셋 때 모델이 튀지 않도록 `CharacterLoader`에서 제거하고, 실제 월드 이동은 `Enemy`의 그룹 이동이 담당합니다. 캐릭터가 하늘에 뜨지 않도록 `Enemy.snapModelToGround()`가 매 프레임 모델 바운딩박스를 바닥에 맞춥니다. Cyclopse의 `Jump.fbx` 추격 모션은 `lockRootVerticalActions: ["chase"]`로 수직 root/hips 이동을 잠가 점프 자세만 취하고, `visualGroundSink: 0.55`로 시각상 발이 바닥에 닿게 전용 접지 보정을 추가합니다.

추격 중 플레이어와의 거리가 `ENEMY_CONFIGS[].giveUpRange`보다 멀어지면 해당 몬스터는 추격을 포기하고 순찰로 돌아갑니다. 층이 다를 때는 계단 우회 거리를 감안해 `interFloorGiveUpRange` 또는 기본 `giveUpRange * 1.8`을 사용합니다. 플레이어가 벽이나 닫힌 문 뒤에 있으면 `Enemy.getChaseTarget()`이 `CollisionWorld.findPath()`로 문 구멍과 `stair-entry-1f`/`stair-exit-2f` waypoint를 통과하는 경로를 잡고, 문 근처에 도착하면 `openDoorOnPath()`가 잠기지 않은 문만 엽니다. 평상시 배회는 `ENEMY_CONFIGS[].patrolWaypointsByFloor`의 현재 층 루프만 사용하고 `allowInterFloor=false`로 pathfinding을 호출하므로, 몬스터가 추격도 아닌데 계단 앞에 튀어나오거나 다른 층으로 순찰 이동하지 않습니다. 계단은 추격 중에만 층간 경로로 사용합니다. 계단이나 문 앞에서 실제 이동량이 거의 없으면 `Enemy.updateStuckState()`가 `tryUnstuck()`을 호출하고, 순찰 중에는 순간이동 대신 다음 waypoint로 넘겨 복도 순찰을 이어갑니다. 순찰 지점에서는 짧게 멈춰 `lookAroundTurnSpeed` 속도로 주변을 훑고, 캐비넷 이벤트가 끝나면 현재 층에서 가장 가까운 순찰 지점으로 재진입한 뒤 복도 순찰을 재개합니다.
