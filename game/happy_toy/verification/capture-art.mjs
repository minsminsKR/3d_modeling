...
const pose = await page.evaluate(() => {
  ...
  game.hud.setStatus("미술실입니다. 물감이 아직 마르지 않았습니다.", 2800);
  return game.poseForCapture({
    x: 128.05,
    y: 0,
    z: -18.4,
    lookAt: [125.6, 1.1, -14.4],
    flashlight: true,
    freezeLoop: true,
  });
});
await page.evaluate(() => window.__happyToy.renderer.render(window.__happyToy.scene, window.__happyToy.camera));
await page.screenshot({ path: path.join(outDir, "f1_art_room.png"), timeout: 120000 });
console.log("art", pose);
await browser.close();
console.log("ok");
