...
const videoDir = path.join(outDir, "center-chase-hide-raw");
...
await page.evaluate(() => {
  ...
  game.player.setPosition({ x: 12.3, y: 0, z: 0 });
...
const poseChase = (uncatX) => page.evaluate((uncatX) => {
  ...
  return {
    ...
    dist: Math.hypot(uncat.group.position.x - 12.3, uncat.group.position.z),
  };
}, uncatX);
...
const hide = await page.evaluate(() => {
  ...
  uncat.setDormant(false);
  uncat.group.visible = true;
  uncat.group.position.set(14.6, 0, 0);
...
if (!hide.hidden) {
  throw new Error(`center-hall hide failed: ${JSON.stringify(hide)}`);
}
...
const stillPath = path.join(outDir, "f1_center_hall_chase_hide_still.png");
...
const dest = path.join(outDir, "f1_center_hall_chase_hide.webm");
