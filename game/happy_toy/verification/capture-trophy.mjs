...
const pose = await page.evaluate(() => {
  ...
  game.hud.setStatus("트로피 복도입니다. 컵은 있는데 이름이 지워져 있습니다.", 2800);
  return game.poseForCapture({
    x: 111.9,
    y: 0,
    z: 0.1,
    lookAt: [107.1, 1.05, 1.55],
    flashlight: true,
    freezeLoop: true,
  });
});
await page.evaluate(() => window.__happyToy.renderer.render(window.__happyToy.scene, window.__happyToy.camera));
await page.screenshot({ path: path.join(outDir, "f1_trophy_hall.png"), timeout: 120000 });
console.log("trophy", pose);
await browser.close();
console.log("ok");
