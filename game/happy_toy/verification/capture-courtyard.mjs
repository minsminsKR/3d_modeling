...
const pose = await page.evaluate(() => {
  ...
  game.hud.setStatus("중정입니다. 난간 너머로 내려가지 마십시오.", 2800);
  return game.poseForCapture({
    x: 80.05,
    y: 0,
    z: 11.15,
    lookAt: [80.2, 0.95, 16.1],
    flashlight: true,
    freezeLoop: true,
  });
});
await page.evaluate(() => window.__happyToy.renderer.render(window.__happyToy.scene, window.__happyToy.camera));
await page.screenshot({ path: path.join(outDir, "f1_courtyard_well.png"), timeout: 120000 });
console.log("courtyard", pose);
await browser.close();
console.log("ok");
