...
require('node:fs').mkdirSync(new URL('./switch-light-qa/',import.meta.url),{recursive:true});
...
try {
  ...
  const flashlight=await page.evaluate(()=>{const g=window.__happyToy;g.poseForCapture({flashlight:true,freezeLoop:true});const intensity=g.flashlight.intensity;g.flashlightController.setEnabled(false,false);g.flashlightController.applyOutput(0);return intensity;});
  assert.ok(flashlight>0&&flashlight<=20,'flashlight should use the reduced output');
  const values=[];
  for(const on of [false,true]) {
    ...
    await page.screenshot({path:fileURLToPath(new URL(`./switch-light-qa/wall-lamp-${on?'on':'off'}.png`,import.meta.url))});
  }
  ...
  await page.screenshot({path:fileURLToPath(new URL('./switch-light-qa/fitted-classroom-door.png',import.meta.url))});
  assert.deepEqual(errors,[]);console.log('LIGHTING / DOOR FIT PASSED',JSON.stringify({off:values[0],on:values[1],flashlight,errors}));
