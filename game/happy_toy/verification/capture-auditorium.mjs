...
const pose = await page.evaluate(() => {
  ...
  game.hud.setStatus("강당입니다. 막이 내려와 있고 객석이 당신을 셉니다.", 2800);
  return game.poseForCapture({
    x: 122.4,
    y: 0,
    z: 16.05,
    lookAt: [128.8, 1.15, 16.1],
    flashlight: true,
    freezeLoop: true,
  });
});
await page.evaluate(() => window.__happyToy.renderer.render(window.__happyToy.scene, window.__happyToy.camera));
await page.screenshot({ path: path.join(outDir, "f1_auditorium_aisle.png"), timeout: 120000 });
console.log("auditorium", pose);
await browser.close();
console.log("ok");
