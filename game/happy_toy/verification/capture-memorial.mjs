...
const pose = await page.evaluate(() => {
  ...
  game.hud.setStatus("기념관입니다. 액자 속 얼굴이 비어 있습니다.", 2800);
  return game.poseForCapture({
    x: 94.2,
    y: 0,
    z: 0.12,
    lookAt: [99.4, 1.2, 1.55],
    flashlight: true,
    freezeLoop: true,
  });
});
await page.evaluate(() => window.__happyToy.renderer.render(window.__happyToy.scene, window.__happyToy.camera));
await page.screenshot({ path: path.join(outDir, "f1_memorial_hall.png"), timeout: 120000 });
console.log("memorial", pose);
await browser.close();
console.log("ok");
