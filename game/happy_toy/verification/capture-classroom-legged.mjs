...
const aisle = await page.evaluate(() => {
  ...
  return game.poseForCapture({
    ...
    y: 0,
    z: 2.2,
    lookAt: [12.5, 0.72, 4.15],
    flashlight: true,
    freezeLoop: true,
  });
});
await page.evaluate(() => window.__happyToy.renderer.render(window.__happyToy.scene, window.__happyToy.camera));
await page.screenshot({ path: path.join(outDir, "f1_classroom_legged_desks.png"), timeout: 120000 });
...
const rows = await page.evaluate(() => {
  ...
  return game.poseForCapture({
    ...
    y: 0,
    z: 3.15,
    lookAt: [12.85, 0.78, 4.55],
    flashlight: true,
    freezeLoop: true,
  });
});
await page.evaluate(() => window.__happyToy.renderer.render(window.__happyToy.scene, window.__happyToy.camera));
await page.screenshot({ path: path.join(outDir, "f1_classroom_legged_rows.png"), timeout: 120000 });
console.log("rows", rows);

await browser.close();
console.log("LEGGED CLASSROOM CAPTURE WRITTEN");
