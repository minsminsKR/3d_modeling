...
const desks = await page.evaluate(() => {
  ...
  return game.poseForCapture({
    ...
    y: 0,
    z: 0.22,
    lookAt: [12.05, 0.98, 3.55],
    flashlight: true,
    freezeLoop: true,
  });
});
await page.evaluate(() => window.__happyToy.renderer.render(window.__happyToy.scene, window.__happyToy.camera));
await page.screenshot({ path: path.join(outDir, "f1_hall_door_classroom_desks.png"), timeout: 120000 });
console.log("desks", desks);

await browser.close();
console.log("HALL DOOR CLASSROOM CAPTURE WRITTEN");
